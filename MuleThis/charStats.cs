...
namespace MuleThis
{
    public partial class PluginCore
    {
        private void initCharStats()
        {
            ...
            Core.ItemSelected += new EventHandler<ItemSelectedEventArgs>(Core_ItemSelected);
        }
        ...
        void Core_ItemSelected(object sender, ItemSelectedEventArgs e)
        {
            WorldObject wo = Core.WorldFilter[e.ItemGuid];
            if (itemIsNpcOrPlayer(wo) && wo.Id != Core.CharacterFilter.Id)
            {
                if (wo.ObjectClass == ObjectClass.Player )
                {
                    MuleTarget oldTarget = targetS;
                    targetS = new MuleTarget(MuleTargetType.AutomaticTarget, wo.Id, wo.Name);
                    onTargetChange(oldTarget);
                }
            }
            else if (itemIsInInventory(wo) && wo.ObjectClass == ObjectClass.Salvage)
            {
                string material = getMaterialName(wo.Values(LongValueKey.Material, 0));
                for (int i = 0; i < materialCho.Count; i++)
                {
                    if (material.Equals(materialCho.Text[i]))
                    {
                        materialCho.Selected = i;
                        break;
                    }
                }
            }
